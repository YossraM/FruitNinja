...
public class Hard {
    ...
    private Hard(Stage stage) {
        this.stage = stage;
    }

    public static Hard getInstance(Stage stage) {
        if (instance == null) {
            instance = new Hard(stage);
        }
        return instance;
    }

    public void prepareScene() throws FileNotFoundException, IOException {
        ...
        Image image = new Image(getClass().getResourceAsStream("dragon.png"));
        BackgroundImage img;
        ...
        root.getChildren().addAll(scr, max, life);

        timeline = new Timeline(new KeyFrame(Duration.millis(2000), (ActionEvent t) -> {
            speed += 500 / 3000;
            ...
            Bomb fbomb = b.getType("fatal bomb");
            fbomb.setProperties();
            fbomb.getImageView().setOnMouseClicked((MouseEvent e) -> {
                SliceFatalBomb slice = new SliceFatalBomb(new GetSliced(), (FatalBomb) fbomb);
                slice.execute();
                cont = new StateContext(new BombSound());
                cont.playSound();
                timeline.stop();
            ...
            Fruit specialf2 = new Pomegrenate();

            specialf2.getImageView().setOnMouseClicked(e -> {
                SlicePom slice = new SlicePom(new GetSliced(), (Pomegrenate) specialf2);
                slice.execute();
                cont = new StateContext(new SplatterSound());
                cont.playSound();
                score.setScore(score.getScore() + 1000);
